...
{
  use \OCA\Redaxo\Toolkit\Traits\UtilTrait;
  use \OCA\Redaxo\Toolkit\Traits\ResponseTrait;
  ...
  public const AUTHENTICATION_REFRESH_INTERVAL_MIN = 10;
    ...
    switch ($setting) {
      ...
      case self::AUTHENTICATION_REFRESH_INTERVAL:
        $newValue = filter_var(
          $value,
          FILTER_VALIDATE_INT,
          ['min_range' => self::AUTHENTICATION_REFRESH_INTERVAL_MIN, ],
        );
        if (empty($newValue)) {
          return self::grumble($this->l->t(
            'Value "%1$s" for setting "%2$s" is either not convertible to integer or out of range (minimum is %d seconds).', [
              $value,
              $setting,
              self::AUTHENTICATION_REFRESH_INTERVAL_MIN,
            ]));
        }
        break;
